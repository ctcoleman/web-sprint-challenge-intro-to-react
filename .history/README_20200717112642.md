...
- React is a development library for building user interfaces for web applications (or offline with React Native, but thats a different story). React uses a virtual DOM (copy of the actual DOM) to note changes that need to be made to the actual DOM and changes only those nodes on the DOM that need changing without 
