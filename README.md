...
---------------
    side effects are actions defined within the component, that are taken each time the React component renders. We sync the effects to happen only when specific states or props change by adding the specific state or prop as the second parameter when calling the useEffect method.
------------------------
